...
2. **修正旧条目用 supersede，不删行**：先写新条目拿 id → `UPDATE memory_entries SET superseded_by='<新id>', updated_at=now() WHERE id='<旧id>' AND superseded_by IS NULL`。**不删除、不归档**（老大 09-12 拍板：写错的直接作废，别搞归档）。动库前先备份。
3. **报条数只报「有效」数**（`superseded_by IS NULL`），另注「已作废」数。**禁止只甩表内总行数**。
