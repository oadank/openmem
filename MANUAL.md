...
| 关 | 拦什么 | 怎么过 |
|---|---|---|
| ① 规矩关 | **你自己** 24h 内没被教过规矩就下笔 | 规矩全文就在这次拦截的返回里（`rules` 字段），看完**直接再写一次**即放行 —— 不用去别处领 |
| ② 查重关 | **你自己** 30min 内没搜过库 | 先 `mh_search(<关键词>, requester="<你的 agent 名>")`，再写一次 |
| ③ 重复关 | 要写的这条库里已有 **≥ 0.95** 相似度的条目 | 改用 `mh_update(id="…", content="<合并后最新版>")`；确实不是一回事就把内容写具体再交一次 |

🔴 **按 agent 各自记账（2026-09-18 老大纠正，别改回去）**：
①② 只看**你自己**有没有领规矩 / 搜过库 —— 别人领过、别人搜过**不算你的**。
「codex 领了规矩，所以 claude 可以随便写」是错的，那等于①关退化成"每天随机抽一个倒霉蛋去学规矩、其余人免考"。
（技术上：判定字段落在 `_gate_state.json` 的 `agents.<你的 source>` 下，全库级 `last_*` 只是统计、不参与判定。）
...
3. **被拦时的返回**里带完整「写入四步」（见 §4 开头），所以吃一次拦截 = 规矩进了一次上下文。
   ⚠️ 注意：`mh_search` **必须带 `requester`** 才记得到你名下（不填就等于没搜）；`mh_write` 的 `source` 是必填，所以①②关都认得你。
